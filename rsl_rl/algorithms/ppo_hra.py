...
from rsl_rl.modules import GaussianNetwork, Network, MultiHeadNetwork
...
class HraPPO(AbstractActorCritic):
    ...
    critic_network: Type[nn.Module] = MultiHeadNetwork # Network
    ...
    def _compute_value_loss(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        if self.recurrent:
            ...
            trajectory_evaluations, trajectory_evaluations_head = self.critic.forward(observations, hidden_state=hidden_states)
        ...
        # This is lambda-return which is equal to R(lambda) = A(o,a)+v(o). 
        #   This implementation is different from SpinningUp where the discounted Monte-Carlo return is used.
